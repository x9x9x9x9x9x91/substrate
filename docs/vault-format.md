...
| `ctx.sheet(title)` | `⇒ Promise<{ model, ev }>` | A sheet fence, parsed and evaluated, so a kind doesn't reimplement the sheet grammar (§5.1). **The parsed table and the evaluated one are separate members**: `model` is the raw fence (`headers`, `rows` as strings), and everything a board draws lives on `ev` — `ev.headers`, `ev.rows` (row-major `(scalar | null)[][]`, positional against `ev.headers`, *not* keyed by column name), `ev.computed` (`{ name, cells }`, one entry per formula column, `cells` parallel to `ev.rows`) and `ev.summaries` (`{ name, value, group }`). Any evaluated value may be `{ err }` instead of a scalar — render the message, or the cell prints `[object Object]`. A computed cell may also be `null`: a formula derives nothing where it read an empty cell through, and a row with nothing typed in it derives nothing at all. |
| `ctx.mounts()` | `⇒ Promise<Mount[]>` | Every mounted folder this vault has (§8) — the roster a built-in board reads when a `source:` names a mount instead of a database. Metadata only: `id`, `name`, `globs`, `ignore?`, `watch?`, `path?`, `missing`, `scanned`, `files` — no rows, and no verb. A copy, so mutating it changes nothing. **An absent `path` is an ordinary state**, not a fault: the folder isn't bound on *this* device, and its index still reads, which is the whole point of a mount that syncs. `missing` is the other one — bound here, and the folder is gone. Added inside api 1, so **feature-check it** (`ctx.mounts?.()`). |
| `ctx.mountRows(name)` | `⇒ Promise<MountRow[]>` | One mount's last-known index rows, by mount name — folded, like every user-authored identity in the vault. These are the rows a chart or a card over that folder draws, out of the same cache: `rel`, `name`, `extension`, `size`, `modified`, `created`, `identity`, `missing?`, the sidecar's `note` path, `props` (the sidecar's merged with what was read out of the file itself — unknown-typed, same coercion rule as `ctx.notes`, and absent until a scan extracts them) and an `excerpt` with its `excerpt_partial` flag. `missing` marks a row the index remembers and the disk no longer has — draw it as absent, never as a file of size zero. **Rejects in two cases that must not be confused with an empty folder**: there is no mount by that name, and the mount is there but its index wouldn't read; the pane shows the same line. Added inside api 1, so **feature-check it**. |
| `ctx.view(name)` | `⇒ Promise<EvaluatedView>` | A saved view — a pin (§7) — evaluated by the app's own evaluator, by name, so a kind that renders "the open tasks" can't drift from the table beside it: the rows, their order, the section headers and every painted cell are what the database pane shows and what the headless reader prints (§7b). **Read-only** — a kind can't create, rename or delete a pin. The name folds, and the **first** pin whose folded name matches answers (two pins may legally share one, and picking one beats refusing); it rejects when no pin carries the name at all. **What you get is always a table**: a pin saved as a board or a calendar evaluates to its rows here, because rows are what a kind can draw. Its own columns, sort keys and grouping apply, and so does the grouping its database carries when the pin captured none. `rows` is the whole sequence in painted order, `groups` is that same sequence cut into the sections a grouped table draws (a view with no grouping has none), and each cell carries `raw` and `display` — **draw `display`**; `raw` is there for comparing and sorting. What does *not* reach a kind is the presentation nothing here could paint anyway: footer aggregations, column widths, wrap. Added inside api 1, so **feature-check it** (`ctx.view?.("Open tasks")`). |
| `ctx.schema()` | `⇒ DbSchema[]` | The vault's databases and their registered properties (§6), so a kind discovers a column's kind and its options instead of hardcoding them — `{ name, props }` per database, each prop `{ name, kind, options, target?, format?, description? }`. **Synchronous, unlike the reads above** — this is already in memory — so call it per draw rather than stashing it: a property registered while the board is open shows up in the next call and not in an old array. What you get is a **projection, not the stored schema file**: the reserved `icon`, `home` and `parent` keys the app keeps in the same map are dropped, because they aren't properties and a kind looping over them would draw a column called "icon". A database with nothing registered still lands, carrying an empty `props` — it exists, and reading it as absent is a different claim — and a property *not* registered here can still be on a note. Added inside api 1, so **feature-check it** (`ctx.schema?.() ?? []`). |
...
**There is no `ctx.move`.** Renaming a note, moving it between folders,
trashing it — none of that is offered, and the absence is a decision rather
than an oversight. It is not a line about how far a kind is trusted, which
would be an odd line to draw when a kind already reads every note and writes
to any of them; it is about what a rename IS in this vault. Renaming a note is
not a write to that note: the engine retargets every wiki-link pointing at it,
the sidebar pins, the shortcut keys, the saved views' sort and filter keys and
the relation props of every other database that names it, and a folder move
drags the folder's own metadata with it. That fan-out is one operation with
one undo entry, and it holds together only because exactly one caller performs
it — the app's own surface, where the undo stack, the open editors and the
index invalidation are all in reach. A second caller with none of that in
reach would be a rename that half-lands, and half-landed is worse than not
offered: the damage is spread across notes nobody was looking at. So the
engine's rename stays behind the app, and a kind that wants a note somewhere
else asks the person — `ctx.openNote(path)` puts the note in front of them,
where the app's own move is one gesture away, and `ctx.toast(msg, action)`
gives that ask a button.
