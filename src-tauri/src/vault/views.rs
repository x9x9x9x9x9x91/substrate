...
fn is_cased(ch: char) -> bool {
    static RE: OnceLock<Regex> = OnceLock::new();
    let mut encoded = [0; 4];
    RE.get_or_init(|| Regex::new(r"^\p{Cased}$").unwrap()).is_match(ch.encode_utf8(&mut encoded))
}
...
impl Engine {
    ...
    pub fn set_view_pref(
        ...
        let hidden_per_layout = hidden_per_layout.and_then(|h| {
            ...
            if h.table.is_none() && h.list.is_none() {
                None
            } else {
                Some(h)
            }
    ...
    pub(super) fn write_folder_meta(
        &self,
        meta: &HashMap<String, FolderMeta>,
    ) -> Result<(), String> {
        let mut map = self.views_file();
        if meta.is_empty() {
    ...
    pub(super) fn move_sidebar_keys(
        &self,
        old_rel: &str,
        new_rel: Option<&str>,
    ) -> Result<(), String> {
    ...
    pub(super) fn move_sidebar_keys_folder(
        &self,
        old_rel: &str,
        new_rel: Option<&str>,
    ) -> Result<(), String> {
        ...
        for v in saved.iter_mut() {
            ...
            if let Some(sorts) = v.sorts.take() {
                ...
                let kept: Vec<SavedViewSort> = match new {
                    Some(n) => sorts
                        ...
                        .collect(),
                    None => sorts.into_iter().filter(|s| !folded_eq(&s.key, old)).collect(),
                };
            ...
            if let Some(cols) = v.columns.take() {
                ...
                let kept: Vec<String> = match new {
                    ...
                    None => cols.into_iter().filter(|c| !folded_eq(c, old)).collect(),
                };
                touched |= had_old;
