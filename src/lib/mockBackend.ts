/* The browser/e2e mock backend — everything the app runs against outside
   Tauri: the whole mock vault state, the `mockDispatch` command switch
   behind `mockInvoke`, the mock event registry behind `mockListen`, and
   the window.__mock* e2e surface installed at module scope. tauri.ts (the
...
export const mockListen: typeof tauriListen = async (event, handler) => {
...
if (!isTauri) {
  ...
  window.__mockSetEchoOnWrites = (on) => {
    mockEchoOnWrites = on;
    if (!on) cancelMockEcho(); // a pending echo dies with the flag
  };
