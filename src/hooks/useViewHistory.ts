...
// a database's open entry lives in `dbNote`, not `selected` (SUB-668), so a
...
/**
 * SUB-392: ⌫ walks back through the views this session visited, restoring the
 * row that was selected when the view was left. Search is a modal detour with
 * its own return flows (Esc, SUB-111/267), so neither side of a search
...
export function useViewHistory(opts: {
  ...
  useEffect(() => {
    ...
    if (prev.view.kind === "search") {
      ...
      // where the search was opened FROM (Esc-return, SUB-267, still owns
  ...
  const goBack = useCallback(() => {
    ...
    // change itself fires (SUB-695) — same one-shot carry a db→db birth uses,
