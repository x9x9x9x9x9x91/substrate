        ...
        <div className="dots-menu db-sorts-menu">
          ...
          {sorts.map((s, i) => (
              <div
                ...
                draggable
                onDragStart={(e) => {
                  // WebKit cancels a drag whose dragstart leaves the data
                  // store empty — the payload itself is unused (dragKey
                  // state is the source of truth), but it has to exist
          ...
          <div className="db-sorts-hint">
            {/* the drag half is a claim about keys, so it waits until there are
                two to reorder; the shift-click half is always true */}
            {sorts.length > 1 && "Drag a key to change its priority. "}
            Shift-click a column header to add a sort key.
          </div>
        </div>
      )}
    </div>
