...
export default function App() {
  ...
  /* Stable identities for the five db-pane handlers that were inline arrows at
     each call site — same reason as the Sidebar/ListPane block above. */
  const openSavedView = useCallback((id: string) => setView({ kind: "saved", id }), [setView]);
  const openSavedViewMenu = useCallback(
    (id: string, x: number, y: number) => setMenu({ x, y, items: savedViewMenuItems(id) }),
    [savedViewMenuItems]
  );
  const onDbRename = useCallback(
    (dbType: string) => setDbDialog({ kind: "rename-db", dbType }),
    [setDbDialog]
  );
  const onDbDelete = useCallback(
    (dbType: string) => setDbDialog({ kind: "delete-db", dbType }),
    [setDbDialog]
  );
  const onDbRenameProp = useCallback(
    (dbType: string, prop: string) => setDbDialog({ kind: "rename-prop", dbType, prop }),
    [setDbDialog]
  );
  const dbPrefType = view.kind === "db" ? view.type : null;
  const dbPrefChange = useCallback(
    (p: ViewPref) => {
      if (dbPrefType) setDbPref(dbPrefType, p);
    },
    [setDbPref, dbPrefType]
  );
  const mountPrefName = activeMount?.name ?? null;
  const mountPrefChange = useCallback(
    (p: ViewPref) => {
      if (mountPrefName) setDbPref(mountPrefName, p);
    },
    [setDbPref, mountPrefName]
  );
  const onTrashMountRows = useCallback(
    () => showToast("A mounted folder's files are only ever read"),
    [showToast]
  );
  const savedId = activeSaved?.id ?? null;
  const savedDb = activeSaved?.db ?? null;
  const savedColumnsChange = useCallback(
    (cols: string[] | undefined) => {
      if (savedId) setViewColumns(savedId, cols);
    },
    [setViewColumns, savedId]
  );
  const openSavedsDb = useCallback(() => {
    if (savedDb) setView({ kind: "db", type: savedDb });
  }, [setView, savedDb]);
  /* `savedViewPref` composes a fresh object every call, so spelling it inline
     at the call site handed the memoized pane a changed `pref` on every render
     — the exact silent-no-op this bite exists to avoid. */
  const savedPref = useMemo(
    () =>
      activeSaved
        ? svPref ?? savedViewPref(activeSaved, byFoldedKey(viewsConfig, activeSaved.db))
        : undefined,
    [activeSaved, svPref, viewsConfig]
  );

  /* The prop belt all three DatabasePane mounts share, gathered once so a new
     shared prop is added in one place instead of three (they used to hold ~35
     identical props each — the file's worst measured conflict magnet). Stable
     identity is the other half of the job: DatabasePane is memoized, and the
     per-database closures DbPaneStack derives from this object are only as
     stable as the object is. */
  const noKeys = useMemo<Record<string, string>>(() => ({}), []);
  const dbPaneCtx = useMemo<DbPaneCtx>(
    () => ({
      allNotes: notes,
      schema,
      savedViews,
      ...
      pinKeys: mobile ? noKeys : pinKeys,
      gridDefault: dbGrid,
      exportRef: dbExportRef,
      onCreateEntry: createEntry,
      onMutated: refresh,
      onToast: showToast,
      onSaveIcon: saveSchemaIcon,
      usedValues,
      onSaveSchema: saveSchemaProp,
      onPromoteOption: promoteSchemaOption,
      onSaveView: saveView,
      onOpenView: openSavedView,
      onViewMenu: openSavedViewMenu,
      onRenameDb: onDbRename,
      onDeleteDb: onDbDelete,
      onRenameProp: onDbRenameProp,
      onRemoveProp: removeProperty,
      onSetParentProp: setDbParentProp,
    }),
    [
      notes,
      schema,
